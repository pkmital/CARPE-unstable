...
{  
  ...
  //<<changchang
  GLenum binding;
  if(target == GL_TEXTURE_RECTANGLE_ARB) binding = GL_TEXTURE_BINDING_RECTANGLE_ARB;
  else if(target == GL_TEXTURE_2D) binding = GL_TEXTURE_BINDING_2D;
  else if(target == GL_TEXTURE_1D) binding = GL_TEXTURE_BINDING_1D;
  else if(target == GL_TEXTURE_3D) binding = GL_TEXTURE_BINDING_3D;
  else binding = 0;

  glGetIntegerv( binding,  &prevTexBind);
...
{  
  ...
  if(prevTexBind!=texture) glBindTexture(target, texture);
  glGetTexLevelParameteriv(target, 0, GL_TEXTURE_WIDTH, &w);
  glGetTexLevelParameteriv(target, 0, GL_TEXTURE_HEIGHT, &h);
  ...
  if(prevTexBind!=texture)  glBindTexture(target, prevTexBind);
}
