...
namespace CommonLib{
  ...
  inline bool printToFile(const Cuda::HostMemoryHeap<int, 2> &data, const char *name = "file.txt"){
    ...
    for (unsigned int y = 0; y < data.size[1]; y++){
      for (unsigned int x = 0; x < data.size[0]; x++){
        fprintf(pFile, "%5d ", data.getBuffer()[y * data.stride[0] + x]);
      }
      fprintf(pFile, "\n");
    }
    fclose (pFile);
  ...
  inline bool printToFile(const Cuda::HostMemoryHeap<int, 1> &data, const char *name = "file.txt"){
    ...
    if (pFile == NULL)
      COMMONLIB_THROW_ERROR("Unable to open file");
    for (unsigned int x = 0; x < data.size[0]; x++)
      fprintf(pFile, "%5d ", data.getBuffer()[x]);
    fclose (pFile);
    return true;
  }
  //-----------------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------------
  //! Fills the given volume with zero values efficiently, by copying subvolumes
  //! @param volume         the volume to clear
  //! @return True if call was successful
  template<typename T>
  inline bool clearVolume(Cuda::DeviceMemory<T, 3>& volume) {
      return iClearVolume(volume);
  }
  ...
  //! @param b              upper limit for random numbers
  //! @return True if call was successful
  inline bool randomNumbers(Cuda::DeviceMemory<float, 3>* data,
  ...
  //-----------------------------------------------------------------------------
  //! Performs gaussian filtering of a floating point 3D device image.
  //!
  //! @todo Maybe a separate input and output image would be desireable.
  //! @param[out] data Dataset to be smoothed
  //! @param[in] sigma Sigma of the convolution kernel
  //! @param[in] kernelsize The size of the convolution kernel. If omitted, a 6-sigma environment is used.
  //! @return True if call was successful
  inline bool gaussianConvolution3D(Cuda::DeviceMemoryPitched<float, 3> *data,
    float sigma, unsigned int kernelsize = 0) {
      return iGaussianConvolution3D(data, sigma, kernelsize);
  }
  ...
  //-----------------------------------------------------------------------------
  //! Applies a 3x3x3 median filter to the input volume that is located on the device.
  //!
  //! Attention: The filter cannot be applied inplace.
  //! @param[in] input   Input image on the device
  //! @param[out] output Filtered image on the device
  //! @param forceXYSliceFilteringOnly Determines if we do the filtering slicewise
  //!                                  in x-y direction only!
  //! @return True if call was successful
  //!
  //! @note \a Datatypes: float \n
  //!       \a Compute \a Capability: TODO \n
  //!       \a ROI: TODO \n
  template<class T>
  inline bool medianFilter3x3x3(const Cuda::DeviceMemoryPitched<T, 3>& input,
                                Cuda::DeviceMemoryPitched<T, 3>& output,
                                const bool forceXYSliceFilteringOnly = false) {
      return iMedianFilter3x3x3(input, output, forceXYSliceFilteringOnly);
  }
  ...
  //-----------------------------------------------------------------------------
  //! Calculates gradients in x, y and z direction, and returns the sqared sum of
  //! these gradients -> uses Simoncelli Derivatives instead of forward differences
  //! @param[in]  input Input image
  //! @param[out] output Edge image
  //! @param[in]  kernelSize size of the kernel (either 3 or 5)
  //! @return True if call was successful
  //!
  //! @note \a Datatypes: float \n
  //!       \a Compute \a Capability: TODO \n
  //!       \a ROI: TODO \n
  template<class T>
  inline bool edgeDetectionSimoncelli( const Cuda::DeviceMemory<T, 3> * input,
                                       Cuda::DeviceMemory<T, 3> *output,
                                       const unsigned int kernelSize)
  {
     return iEdgeDetectionSimoncelli3D( input, output, kernelSize );
  }
  ...
  //! @param[in] absolute_intensity_range_low Lower value of the intensity range included
  //!                                         into the region grow area
  //! @param[in] absolute_intensity_range_high Higher value of the intensity range included
   //!                                         into the region grow area
  //! @return True if call was successful
  //!
  ...
  inline bool regionGrowing( Cuda::DeviceMemory<T, 3> *region,
    Cuda::DeviceMemory<U, 3> *image, U absolute_intensity_range_low,
    U absolute_intensity_range_high)
  {
    return iRegionGrowing(region, image, absolute_intensity_range_low, absolute_intensity_range_high);
  }
    ...
    int line_width, T intensity)
  {
    ...
    //-----------------------------------------------------------------------------
  //! Draws a sphere of specified size to a 3D image
  //! @param[out] image  Input image to which will be drawn
  //! @param[in] x_center Center coordinate x
  //! @param[in] y_center Center coordinate y
  //! @param[in] z_center Center coordinate z
  //! @param[in] radius Radius of sphere (physical, consider spacing!)
  //! @param[in] intensity intensity with which will be drawn
  //! @return True if call was successful
  //!
  //! @note \a Datatypes: float \n
  ...
  //!       \a ROI: TODO \n
  template<class T>
  inline bool drawSphere( Cuda::DeviceMemory<T, 3> *image,
    int x_center, int y_center, int z_center,
    int radius, T intensity)
  {
    return iDrawSphere3D(image, x_center, y_center, z_center, radius,  intensity);
  }
