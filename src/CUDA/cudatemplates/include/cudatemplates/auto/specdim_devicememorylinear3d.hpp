...
#ifndef CUDA_DEVICEMEMORYLINEAR3D_H
#define CUDA_DEVICEMEMORYLINEAR3D_H
...
public:
  ...
  */
  inline DeviceMemoryLinear3D(const Size<3> &_size):
    Layout<Type, 3>(_size),
    Pointer<Type, 3>(_size),
    DeviceMemoryLinear<Type, 3>(_size)
  {
  }
  ...
  */
  inline DeviceMemoryLinear3D(const Layout<Type, 3> &layout):
    Layout<Type, 3>(layout),
    Pointer<Type, 3>(layout),
    DeviceMemoryLinear<Type, 3>(layout)
  {
  }
  ...
  /**
     Constructor.
  */
  inline DeviceMemoryLinear3D(size_t size0, size_t size1, size_t size2):
    Layout<Type, 3>(Size<3>(size0, size1, size2)),
    Pointer<Type, 3>(Size<3>(size0, size1, size2)),
    DeviceMemoryLinear<Type, 3>(Size<3>(size0, size1, size2))
  {
  }

  /**
     Copy constructor.
     @param x instance of DeviceMemoryLinear3D to be copied
  */
  inline DeviceMemoryLinear3D(const DeviceMemoryLinear3D<Type> &x):
    Layout<Type, 3>(x),
    Pointer<Type, 3>(x),
    DeviceMemoryLinear<Type, 3>(x)
  {
  }
  ...
  */
  template<class Name>
    inline DeviceMemoryLinear3D(const Name &x):
    Layout<Type, 3>(x),
    Pointer<Type, 3>(x),
    DeviceMemoryLinear<Type, 3>(x)
  {
  ...
  */
  template<class Name>
    inline DeviceMemoryLinear3D(const Name &x, const Size<3> &ofs, const Size<3> &size):
    Layout<Type, 3>(size),
    Pointer<Type, 3>(size),
    DeviceMemoryLinear<Type, 3>(x, ofs, size)
  {
  }
  ...
  */
  template<class Name>
    inline DeviceMemoryLinear3D(const Name &x, size_t ofs0, size_t ofs1, size_t ofs2, size_t size0, size_t size1, size_t size2):
    Layout<Type, 3>(Size<3>(size0, size1, size2)),
    Pointer<Type, 3>(Size<3>(size0, size1, size2)),
    DeviceMemoryLinear<Type, 3>(x, Size<3>(ofs0, ofs1, ofs2), Size<3>(size0, size1, size2))
  {
  }
  ...
  */
  inline void alloc()
  {
    DeviceMemoryLinear<Type, 3>::alloc();
  }
