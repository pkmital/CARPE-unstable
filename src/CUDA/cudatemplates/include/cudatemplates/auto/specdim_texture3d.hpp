...
#ifndef CUDA_TEXTURE3D_H
#define CUDA_TEXTURE3D_H
...
/**
   Texture template specialized for 3 dimension(s).
*/
template <class Type>
class Texture3D:
    virtual public Layout<Type, 3>,
    virtual public Pointer<Type, 3>,
    public Texture<Type, 3>
{
public:
  ...
  */
  inline Texture3D(const Size<3> &_size):
    Layout<Type, 3>(_size),
    Pointer<Type, 3>(_size),
    Texture<Type, 3>(_size)
  {
  }
  ...
  */
  inline Texture3D(const Layout<Type, 3> &layout):
    Layout<Type, 3>(layout),
    Pointer<Type, 3>(layout),
    Texture<Type, 3>(layout)
  {
  }
  ...
  /**
     Constructor.
  */
  inline Texture3D(size_t size0, size_t size1, size_t size2):
    Layout<Type, 3>(Size<3>(size0, size1, size2)),
    Pointer<Type, 3>(Size<3>(size0, size1, size2)),
    Texture<Type, 3>(Size<3>(size0, size1, size2))
  {
  }

  /**
     Copy constructor.
     @param x instance of Texture3D to be copied
  */
  inline Texture3D(const Texture3D<Type> &x):
    Layout<Type, 3>(x),
    Pointer<Type, 3>(x),
    Texture<Type, 3>(x)
  {
  ...
  */
  template<class Name>
    inline Texture3D(const Name &x, const Size<3> &ofs, const Size<3> &size):
    Layout<Type, 3>(size),
    Pointer<Type, 3>(size),
    Texture<Type, 3>(x, ofs, size)
  {
  }
  ...
  */
  template<class Name>
    inline Texture3D(const Name &x, size_t ofs0, size_t ofs1, size_t ofs2, size_t size0, size_t size1, size_t size2):
    Layout<Type, 3>(Size<3>(size0, size1, size2)),
    Pointer<Type, 3>(Size<3>(size0, size1, size2)),
    Texture<Type, 3>(x, Size<3>(ofs0, ofs1, ofs2), Size<3>(size0, size1, size2))
  {
  }
