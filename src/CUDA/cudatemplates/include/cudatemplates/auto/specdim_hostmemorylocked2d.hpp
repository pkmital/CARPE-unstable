...
#ifndef CUDA_HOSTMEMORYLOCKED2D_H
#define CUDA_HOSTMEMORYLOCKED2D_H
...
class HostMemoryLocked2D:
    ...
    public HostMemoryLocked<Type, 2>
{
public:
  ...
  */
  inline HostMemoryLocked2D(const Size<2> &_size):
    Layout<Type, 2>(_size),
    Pointer<Type, 2>(_size),
    HostMemoryLocked<Type, 2>(_size)
  {
  }
  ...
  */
  inline HostMemoryLocked2D(const Layout<Type, 2> &layout):
    Layout<Type, 2>(layout),
    Pointer<Type, 2>(layout),
    HostMemoryLocked<Type, 2>(layout)
  {
  }
  ...
  /**
     Constructor.
  */
  inline HostMemoryLocked2D(size_t size0, size_t size1):
    Layout<Type, 2>(Size<2>(size0, size1)),
    Pointer<Type, 2>(Size<2>(size0, size1)),
    HostMemoryLocked<Type, 2>(Size<2>(size0, size1))
  {
  }

  /**
     Copy constructor.
     @param x instance of HostMemoryLocked2D to be copied
  */
  inline HostMemoryLocked2D(const HostMemoryLocked2D<Type> &x):
    Layout<Type, 2>(x),
    Pointer<Type, 2>(x),
    HostMemoryLocked<Type, 2>(x)
  {
  }
  ...
  */
  template<class Name>
    inline HostMemoryLocked2D(const Name &x):
    Layout<Type, 2>(x),
    Pointer<Type, 2>(x),
    HostMemoryLocked<Type, 2>(x)
  {
  ...
  */
  template<class Name>
    inline HostMemoryLocked2D(const Name &x, const Size<2> &ofs, const Size<2> &size):
    Layout<Type, 2>(size),
    Pointer<Type, 2>(size),
    HostMemoryLocked<Type, 2>(x, ofs, size)
  {
  }
  ...
  */
  template<class Name>
    inline HostMemoryLocked2D(const Name &x, size_t ofs0, size_t ofs1, size_t size0, size_t size1):
    Layout<Type, 2>(Size<2>(size0, size1)),
    Pointer<Type, 2>(Size<2>(size0, size1)),
    HostMemoryLocked<Type, 2>(x, Size<2>(ofs0, ofs1), Size<2>(size0, size1))
  {
  }
