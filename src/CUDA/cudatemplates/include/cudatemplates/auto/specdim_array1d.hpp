...
#ifndef CUDA_ARRAY1D_H
#define CUDA_ARRAY1D_H
...
/**
   Array template specialized for 1 dimension(s).
*/
template <class Type>
class Array1D:
...
public:
  ...
  */
  inline Array1D(const Size<1> &_size):
    Layout<Type, 1>(_size),
  ...
  */
  inline Array1D(const Layout<Type, 1> &layout):
  ...
  /**
     Constructor.
  */
  inline Array1D(size_t size0):
    Layout<Type, 1>(Size<1>(size0)),
    // Pointer<Type, 1>(Size<1>(size0)),
    Array<Type, 1>(Size<1>(size0))
  {
  }

  /**
     Copy constructor.
     @param x instance of Array1D to be copied
  */
  inline Array1D(const Array1D<Type> &x):
    Layout<Type, 1>(x),
    // Pointer<Type, 1>(x),
    Array<Type, 1>(x)
  {
  ...
  */
  template<class Name>
    inline Array1D(const Name &x, const Size<1> &ofs, const Size<1> &size):
    Layout<Type, 1>(size),
    // Pointer<Type, 1>(size),
    Array<Type, 1>(x, ofs, size)
  {
  }
  ...
  */
  template<class Name>
    inline Array1D(const Name &x, size_t ofs0, size_t size0):
    Layout<Type, 1>(Size<1>(size0)),
    // Pointer<Type, 1>(Size<1>(size0)),
    Array<Type, 1>(x, Size<1>(ofs0), Size<1>(size0))
  {
  }
