...
#ifndef CUDA_HOSTMEMORYLOCKED3D_H
#define CUDA_HOSTMEMORYLOCKED3D_H
...
public:
  ...
  */
  inline HostMemoryLocked3D(const Size<3> &_size):
    Layout<Type, 3>(_size),
    Pointer<Type, 3>(_size),
    HostMemoryLocked<Type, 3>(_size)
  {
  }
  ...
  */
  inline HostMemoryLocked3D(const Layout<Type, 3> &layout):
    Layout<Type, 3>(layout),
    Pointer<Type, 3>(layout),
    HostMemoryLocked<Type, 3>(layout)
  {
  }
  ...
  /**
     Constructor.
  */
  inline HostMemoryLocked3D(size_t size0, size_t size1, size_t size2):
    Layout<Type, 3>(Size<3>(size0, size1, size2)),
    Pointer<Type, 3>(Size<3>(size0, size1, size2)),
    HostMemoryLocked<Type, 3>(Size<3>(size0, size1, size2))
  {
  }

  /**
     Copy constructor.
     @param x instance of HostMemoryLocked3D to be copied
  */
  inline HostMemoryLocked3D(const HostMemoryLocked3D<Type> &x):
    Layout<Type, 3>(x),
    Pointer<Type, 3>(x),
    HostMemoryLocked<Type, 3>(x)
  {
  }
  ...
  */
  template<class Name>
    inline HostMemoryLocked3D(const Name &x):
    Layout<Type, 3>(x),
    Pointer<Type, 3>(x),
    HostMemoryLocked<Type, 3>(x)
  {
  ...
  */
  template<class Name>
    inline HostMemoryLocked3D(const Name &x, const Size<3> &ofs, const Size<3> &size):
    Layout<Type, 3>(size),
    Pointer<Type, 3>(size),
    HostMemoryLocked<Type, 3>(x, ofs, size)
  {
  }
  ...
  */
  template<class Name>
    inline HostMemoryLocked3D(const Name &x, size_t ofs0, size_t ofs1, size_t ofs2, size_t size0, size_t size1, size_t size2):
    Layout<Type, 3>(Size<3>(size0, size1, size2)),
    Pointer<Type, 3>(Size<3>(size0, size1, size2)),
