...
#ifndef CUDA_SYMBOL1D_H
#define CUDA_SYMBOL1D_H
...
public:
  ...
  */
  inline Symbol1D(const Size<1> &_size):
    Layout<Type, 1>(_size),
  ...
  */
  inline Symbol1D(const Layout<Type, 1> &layout):
    Layout<Type, 1>(layout),
    Pointer<Type, 1>(layout),
    Symbol<Type, 1>(layout)
  {
  }
  ...
  /**
     Constructor.
  */
  inline Symbol1D(size_t size0):
    Layout<Type, 1>(Size<1>(size0)),
    Pointer<Type, 1>(Size<1>(size0)),
    Symbol<Type, 1>(Size<1>(size0))
  {
  }

  /**
     Copy constructor.
     @param x instance of Symbol1D to be copied
  */
  inline Symbol1D(const Symbol1D<Type> &x):
    Layout<Type, 1>(x),
    Pointer<Type, 1>(x),
    Symbol<Type, 1>(x)
  {
  ...
  */
  template<class Name>
    inline Symbol1D(const Name &x, const Size<1> &ofs, const Size<1> &size):
    Layout<Type, 1>(size),
    Pointer<Type, 1>(size),
    Symbol<Type, 1>(x, ofs, size)
  {
  }
  ...
  */
  template<class Name>
    inline Symbol1D(const Name &x, size_t ofs0, size_t size0):
    Layout<Type, 1>(Size<1>(size0)),
    Pointer<Type, 1>(Size<1>(size0)),
