...
#ifndef __OFXGAUSSIANMIXTUREMODEL
#define __OFXGAUSSIANMIXTUREMODEL
...
class ofxGaussianMixtureModel
{
enum {COV_SPHERICAL, COV_DIAGONAL, COV_GENERIC};
public:
	// setup the mixture model (variables has to be 2)
	ofxGaussianMixtureModel(double *inputData, int observations, int variables, int map_scalar = 1, int cov_type = COV_SPHERICAL);

	// release CvMats stuff...
	~ofxGaussianMixtureModel();
